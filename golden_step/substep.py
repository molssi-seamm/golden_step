...
"""Base class for substeps in the Golden step."""
...
class Substep(seamm.Node):
    """A base class for substeps in the Golden subflowchart.

    The top-level Golden step iterates over the substeps
    of its subflowchart and calls each substep's `run()` method. Each substep
    is responsible for preparing its own input, invoking Golden,
    ...
    * `version` / `git_revision` - delegated to the plug-in package so that
    ...
    @property
    def version(self):
        """The semantic version of the golden_step package."""
        return golden_step.__version__

    @property
    def git_revision(self):
        """The git revision of the golden_step package."""
    ...
    @property
    def options(self):
        """The Golden options, delegated to the parent step."""
        return self.parent.options

    @property
    def input_only(self):
        """Whether to write the input only, not run Golden."""
